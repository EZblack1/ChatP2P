...
public class ChatP2P {public static void main(String[] args) {
    ...
    private static void enviarMensagens(String endereco, int porta) {
        try (Socket socket = new Socket(endereco, porta);
             PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
             BufferedReader consoleInput = new BufferedReader(new InputStreamReader(System.in))) {

            System.out.println("Conectado ao outro cliente. Digite suas mensagens:");
            System.out.println("Digite 'sair' para encerrar a conversa.");

            String mensagem;
            while ((mensagem = consoleInput.readLine()) != null && !mensagem.equalsIgnoreCase("sair")) {
